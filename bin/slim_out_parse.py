...
def parse_genome_data(genome_data):
    ...
    for curr_indiv in genome_data:
        print(curr_indiv, file = sys.stderr)
        if not curr_indiv.strip().startswith('p'):
            continue
...
def make_haplotype_df(genome_dict, curr_seg_df, hxb2):
    ...
    Returns
    -----------
    haplosome_df:   pd.DataFrame, a dataframe where each row is an individual
                    and each column is a position that is segregating in the
                    population. The values are the alleles that individual has
                    at that position.
    """
    #Make the dataframe we will return
    haplosome_df = []
    ...
    for individual in genome_dict.keys():
        ...
        for mutation in curr_haplotype:
            ...
            #Some mutations are not in the segregating dataframe because they didn't
            #reach the frequency cutoff. We will just skip those mutations
    ...
    #Get the positions that are segregating
    seg_positions = curr_seg_df['position'].unique()
    seg_positions.sort()
    seg_positions = seg_positions.tolist()

    #Now loop through each individual and make a row for the dataframe
    for individual in genome_dict.keys():
        #Get the individual's mutation dictionary
        curr_mut_dict = genotype_dict[individual]
        #Make a list to store the individual's haplotype
        curr_haplotype = []

        #Loop through each segregating position and get the allele
        for position in seg_positions:
            if position in curr_mut_dict:
                curr_haplotype.append(curr_mut_dict[position])
            else:
                curr_haplotype.append(hxb2[position])
        
        #Add the individual's haplotype to the dataframe
        haplosome_df.append([individual] + curr_haplotype)
    haplosome_df = pd.DataFrame(haplosome_df, columns = ['individual'] + seg_positions)
